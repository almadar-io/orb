---
id: std-classifier-game
title: "std-classifier-game"
sidebar_label: "std-classifier-game"
description: "ClassifierGameOrbital behavior with 3 states, 5 events, and 5 transitions."
---

import OrbPreviewBlock from '@shared/OrbPreviewBlock';

# std-classifier-game

**Level**: Molecule | **Entity**: ClassifierGame | **Persistence**: runtime

3 states, 5 events, 5 transitions

## Live Preview

<OrbPreviewBlock schema={`{
  "name": "ClassifierGameOrbital",
  "orbitals": [
    {
      "name": "ClassifierGameOrbital",
      "entity": {
        "name": "ClassifierGame",
        "persistence": "runtime",
        "fields": [
          {
            "name": "id",
            "type": "string"
          },
          {
            "name": "name",
            "type": "string"
          },
          {
            "name": "description",
            "type": "string"
          },
          {
            "name": "status",
            "type": "string",
            "default": "active",
            "values": [
              "active",
              "inactive",
              "pending"
            ]
          },
          {
            "name": "createdAt",
            "type": "string"
          },
          {
            "name": "score",
            "type": "number",
            "default": 0
          },
          {
            "name": "level",
            "type": "number",
            "default": 1
          }
        ],
        "instances": [
          {
            "id": "game-1",
            "name": "Classifier Session",
            "description": "Active game session",
            "status": "active",
            "score": 0,
            "level": 1
          }
        ]
      },
      "traits": [
        {
          "name": "ClassifierGameClassifierGame",
          "linkedEntity": "ClassifierGame",
          "category": "interaction",
          "stateMachine": {
            "states": [
              {
                "name": "menu",
                "isInitial": true
              },
              {
                "name": "playing"
              },
              {
                "name": "complete"
              }
            ],
            "events": [
              {
                "key": "INIT",
                "name": "Initialize"
              },
              {
                "key": "START",
                "name": "Start"
              },
              {
                "key": "COMPLETE",
                "name": "Complete"
              },
              {
                "key": "RESTART",
                "name": "Restart"
              },
              {
                "key": "NAVIGATE",
                "name": "Navigate"
              }
            ],
            "transitions": [
              {
                "from": "menu",
                "to": "menu",
                "event": "INIT",
                "effects": [
                  [
                    "fetch",
                    "ClassifierGame"
                  ],
                  [
                    "render-ui",
                    "main",
                    {
                      "type": "game-menu",
                      "title": "Sort & Classify",
                      "menuItems": [
                        {
                          "label": "Start",
                          "event": "START",
                          "variant": "primary"
                        }
                      ]
                    }
                  ]
                ]
              },
              {
                "from": "menu",
                "to": "playing",
                "event": "START",
                "effects": [
                  [
                    "render-ui",
                    "main",
                    {
                      "type": "stack",
                      "direction": "vertical",
                      "gap": "md",
                      "children": [
                        {
                          "type": "game-hud",
                          "stats": [
                            {
                              "label": "Score",
                              "value": "@entity.score"
                            },
                            {
                              "label": "Level",
                              "value": "@entity.level"
                            }
                          ]
                        },
                        {
                          "type": "classifier-board",
                          "entity": "ClassifierGame",
                          "completeEvent": "COMPLETE"
                        }
                      ]
                    }
                  ]
                ]
              },
              {
                "from": "menu",
                "to": "menu",
                "event": "NAVIGATE",
                "effects": []
              },
              {
                "from": "playing",
                "to": "complete",
                "event": "COMPLETE",
                "effects": [
                  [
                    "render-ui",
                    "main",
                    {
                      "type": "game-over-screen",
                      "title": "Well Done!",
                      "menuItems": [
                        {
                          "label": "Play Again",
                          "event": "RESTART",
                          "variant": "primary"
                        }
                      ]
                    }
                  ]
                ]
              },
              {
                "from": "complete",
                "to": "menu",
                "event": "RESTART",
                "effects": [
                  [
                    "render-ui",
                    "main",
                    {
                      "type": "game-menu",
                      "title": "Sort & Classify",
                      "menuItems": [
                        {
                          "label": "Start",
                          "event": "START",
                          "variant": "primary"
                        }
                      ]
                    }
                  ]
                ]
              }
            ]
          }
        }
      ],
      "pages": [
        {
          "name": "ClassifierGamePage",
          "path": "/classifiergames",
          "traits": [
            {
              "ref": "ClassifierGameClassifierGame"
            }
          ]
        }
      ]
    }
  ],
  "description": "Educational game molecule: menu -> playing -> complete. Uses the \`classifier-board\` pattern for the playing state."
}`} height="450px" />

## Entity Fields

| Field | Type | Default |
|-------|------|---------|
| id | string | - |
| name | string | - |
| description | string | - |
| status | string | `"active"` |
| createdAt | string | - |
| score | number | `0` |
| level | number | `1` |

## States

| State | Type |
|-------|------|
| menu | Initial |
| playing | - |
| complete | - |

## Events

| Event | Payload |
|-------|---------|
| INIT | - |
| START | - |
| COMPLETE | - |
| RESTART | - |
| NAVIGATE | - |

## Transitions

| From | Event | To | Effects |
|------|-------|----|---------|
| menu | INIT | menu | 2 effects |
| menu | START | playing | 1 effect |
| menu | NAVIGATE | menu | - |
| playing | COMPLETE | complete | 1 effect |
| complete | RESTART | menu | 1 effect |
