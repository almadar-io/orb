...
# Install via npm (fallback when binary release is not available)
install_via_npm() {
  ...
  if ! command -v node > /dev/null 2>&1; then
    echo "${YELLOW}Node.js is not installed.${NC}"
    echo ""
    echo "Install Node.js first, then run:"
    echo "  ${GREEN}npm install -g @almadar/orb${NC}"
    echo ""
    echo "Quick install options:"
    echo "  macOS:   brew install node"
    echo "  Ubuntu:  sudo apt install nodejs npm"
    echo "  Any:     curl -o- https://raw.githubusercontent.com/nvm-sh/nvm/v0.40.3/install.sh | bash"
    echo "           nvm install 22"
    exit 1
  fi
  ...
  if ! command -v npm > /dev/null 2>&1; then
    echo "${RED}npm not found. Install npm and try again.${NC}"
    exit 1
  fi

  echo "Installing @almadar/orb via npm..."
  npm install -g @almadar/orb

  if command -v orb > /dev/null 2>&1; then
    echo ""
    echo "${GREEN}Orb CLI installed successfully via npm!${NC}"
    orb --version 2>/dev/null || true
  else
    echo ""
    echo "${RED}Installation completed but 'orb' command not found in PATH.${NC}"
    echo "Try: npx @almadar/orb --help"
  fi
}
...
install() {
  ...
  if [ ! -f "$TMP_DIR/$FILENAME" ]; then
    echo "${YELLOW}Binary release not available for ${PLATFORM_ARCH}${NC}"
    echo ""
    install_via_npm
    return
  fi
